...
namespace Renderer {
				   ...
				   , m_rotation(rotation)
	{
		...
		glVertexAttribPointer(0, 2, GL_FLOAT, GL_FALSE, 0, nullptr);//связываем данные

		glGenBuffers(1, &m_textureCoordsVBO);
		glBindBuffer(GL_ARRAY_BUFFER, m_textureCoordsVBO);
		glBufferData(GL_ARRAY_BUFFER, sizeof(textureCoords), &textureCoords, GL_STATIC_DRAW);
		glEnableVertexAttribArray(1);//функция включает нулевую позицию в шейдере
		glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, 0, nullptr);//связываем данные
