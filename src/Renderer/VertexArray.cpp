...
namespace RenderEngine {
	...
	void VertexArray::addBuffer(const VertexBuffer& vertexBuffer, const VertexBufferLayout& layout) 
	{
		...
		for (unsigned int i = 0; i < layoutElements.size(); i++) {
			...
			glEnableVertexAttribArray(currentAttribIndex);//функция включает нулевую позицию в шейдере
			glVertexAttribPointer(currentAttribIndex, currentLayoutElement.count, currentLayoutElement.type, currentLayoutElement.normalized, layout.getStride(), offset);//связываем данные
