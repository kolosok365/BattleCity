...
Level::Level(const std::vector<std::string>& levelDescription) {
	...
	for (const std::string& currentRow : levelDescription){
		...
		for (const char currentElement : currentRow) {
			...
			m_levelObjects.emplace_back(createGameObjectFromDescription(currentElement, glm::vec2(currentLeftOffset, currentBottomOffset), glm::vec2(BLOCK_SIZE, BLOCK_SIZE),0));
...
size_t Level::gelLevelWidth() const {
	return (m_width + 3) * BLOCK_SIZE;
}

size_t Level::gelLevelHeight() const {
